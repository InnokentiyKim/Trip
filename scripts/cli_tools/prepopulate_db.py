...
async def _load_permissions() -> None:
    ...
    if not sql_file.exists():
        typer.secho(f"Warning: {sql_file} not found, skipping permissions load", fg=typer.colors.YELLOW, bold=True)
        return
    ...
    async with async_container() as request_container:
        ...
        async with engine.begin() as conn:
            ...
            try:
                ...
                typer.secho(f"Error loading permissions", fg=typer.colors.RED)
                raise exc
    ...
    async with async_container() as request_container:
        ...
        async with AsyncSession(engine) as session:
            result = await session.execute(
                text("SELECT id FROM roles WHERE name = 'manager'")
            )
            manager_role_id = result.scalar_one_or_none()

            if not manager_role_id:
                typer.secho(f"Error: Manager role not found", fg=typer.colors.RED, bold=True)
                return
            ...
            try:
                ...
                hotels_data = [
                    {
                        ...
                        "services": {"wifi": True, "parking": True, "pool": True, "gym": True}
                    },
                    {
                        "name": "Seaside Resort",
                        "location": "Sochi, Beach Boulevard",
                        "services": {"wifi": True, "beach": True, "spa": True, "restaurant": True}
                    },
                    {
                        "name": "Mountain View Lodge",
                        "location": "Krasnaya Polyana, Mountain Road",
                        "services": {"wifi": True, "ski": True, "sauna": True, "bar": True}
                    }
                ]

                rooms_templates = [
                    {"name": "Standard Room", "price": Decimal("2500.00"), "services": {"wifi": True, "tv": True}},
                    {"name": "Deluxe Room", "price": Decimal("4000.00"), "services": {"wifi": True, "tv": True, "minibar": True}},
                    {"name": "Suite", "price": Decimal("7500.00"), "services": {"wifi": True, "tv": True, "minibar": True, "jacuzzi": True}},
                    {"name": "Family Room", "price": Decimal("5500.00"), "services": {"wifi": True, "tv": True, "kitchen": True}},
                    {"name": "Presidential Suite", "price": Decimal("15000.00"), "services": {"wifi": True, "tv": True, "minibar": True, "jacuzzi": True, "balcony": True}}
                ]

                for manager, hotel_data in zip(managers, hotels_data):
                    hotel = Hotel(
                ...
                typer.secho(f"Sample data already exists in database. Skipping...", fg=typer.colors.YELLOW)
