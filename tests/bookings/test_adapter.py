...
from tests.fixtures.mocks import MockUser, MockHotel, MockRoom, MockBooking
...
class TestBookingAdapter:
    ...
    async def test_get_bookings_by_status(
        self, booking_adapter, user, sample_booking
    ):
        """Test getting bookings by status."""
        result = await booking_adapter.get_bookings(
    ...
    async def test_get_active_bookings(
        self, booking_adapter, user, sample_booking, confirmed_booking
    ):
    ...
    async def test_get_free_rooms_left(
        self, booking_adapter, sample_room, sample_booking
    ):
    ...
    async def test_add_booking_no_rooms_available(
        self, booking_adapter, user, sample_room
    ):
    ...
    async def test_update_booking_only_active(
        self, booking_adapter, cancelled_booking
    ):
    ...
    async def test_get_bookings_with_date_filter(
        self, booking_adapter, user, bookings
    ):
        """Test getting bookings with date filter."""
        today = date.today()
        result = await booking_adapter.get_bookings(
            user_id=user.id, date_from=today + timedelta(days=7)
        )
