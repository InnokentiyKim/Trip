...
from src.apps.authentication.user.application.exceptions import Unauthorized, UserNotFoundError
...
async def get_info(
    ...
    role_info = await role_management.get_role_info(fetch=fetches.GetRoleInfo(role_id=user_info.role))

    response = UserInfoResponseDTO(
        id=user_info.id,
        email=user_info.email,
        is_mfa_enabled=user_info.auth_info.is_mfa_enabled,
        user_type=role_info.name,
