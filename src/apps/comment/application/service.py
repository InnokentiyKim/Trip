...
from src.apps.comment.domain.commands import (
    AddCommentCommand,
    DeleteCommentCommand,
    UpdateCommentInfoCommand,
)
from src.apps.comment.domain.excepitions import CommentNotFoundError
from src.apps.comment.domain.fetches import (
    GetCommentInfo,
    ListHotelComments,
    ListUserComments,
)
...
class CommentService(ServiceBase):
    ...
    async def add_comment(self, cmd: AddCommentCommand) -> UUID:
        ...
        self._logger.info("New comment added", id=comment.id, hotel_id=hotel.id, user_id=user.id)
    ...
    async def update_comment_info(self, cmd: UpdateCommentInfoCommand) -> None:
        ...
        updating_params = cmd.model_dump(exclude={"comment_id"}, exclude_unset=True)
