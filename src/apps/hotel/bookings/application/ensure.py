...
class BookingServiceEnsurance(ServiceEnsuranceBase):
    ...
    async def booking_exists(self, booking_id: UUID, user_id: int | UUID) -> Booking:
        """Ensure that a booking exists by its ID."""
        ...
        if booking is None:
            ...
            raise exceptions.BookingNotFoundError
        return booking

    async def active_booking_exists(self, booking_id: UUID, user_id: int | UUID) -> Booking:
        """Ensure that an active booking exists by its ID."""
        booking: Booking | None = await self._booking.get_booking_by_id(booking_id, user_id=user_id)
